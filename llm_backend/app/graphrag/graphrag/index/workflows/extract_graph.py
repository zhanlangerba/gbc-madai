...
async def get_summarized_entities_relationships(
    ...
    # 从 extracted_relationships 中删除 description 列
    # 将 relationship_summaries 中的 description 列与 extracted_relationships 中的 source 和 target 列进行合并
    relationships = extracted_relationships.drop(columns=["description"]).merge(
        relationship_summaries, on=["source", "target"], how="left"
    )

    # 从 extracted_entities 中删除 description 列
    # 将 entity_summaries 中的 description 列与 extracted_entities 中的 title 列进行合并
